		...
		$(window).on('scroll',function(){
			if(checkScrollSlide){
				$.each(dataInt.data,function(key,value){
					...
					var fSpan = $("<span>").append("￥·128").appendTo($(foot_money));
					var fa=$('<a>').attr('href','#').addClass('foot_a').appendTo($(foot_money));
					var faSpan=$('<span>').addClass('glyphicon lyphicon glyphicon-heart-empty').append("99").appendTo($(fa));
					
					var foot_title=$('<div>').addClass('foot_title').appendTo($(oBox));
					var fp = $("<p>").append("。123").appendTo($(foot_title));
